...
for i in range(0, len(omegas)):
    ...
    if(i == 0):
        SigIdeal = LineUnwrapIdeal - LineUnwrapIdealBase
        SigSim = LineUnwrapSim - LineUnwrapSimBase
    else:
        SigIdeal = np.vstack((SigIdeal, LineUnwrapIdeal - LineUnwrapIdealBase))
        SigSim = np.vstack((SigSim, LineUnwrapSim - LineUnwrapSimBase))
        
fig = plt.gcf()
plt.contourf(LineYs, omegas, SigIdeal, 100, cmap = cmx.jet)
plt.colorbar()
plt.title('Monpole Ideal Response')
plt.xlabel('y-coordinate (u)')
plt.ylabel('Omega (rad/s)')
plt.savefig("IdealMonopole.pdf", bbox_inches='tight')
plt.show()

fig = plt.gcf()
plt.contourf(LineYs, omegas, SigSim, 100, cmap = cmx.jet)
plt.colorbar()
plt.title('Monpole Sim Response')
plt.xlabel('y-coordinate (u)')
plt.ylabel('Omega (rad/s)')
plt.savefig("SimMonopole.pdf", bbox_inches='tight')
plt.show()

fig = plt.gcf()
plt.contourf(LineYs, omegas, SigSim - SigIdeal, 100, cmap = cmx.jet)
plt.colorbar()
plt.title('Monpole Model Error')
plt.xlabel('y-coordinate (u)')
plt.ylabel('Omega (rad/s)')
plt.savefig("MonopoleError.pdf", bbox_inches='tight')
plt.show()