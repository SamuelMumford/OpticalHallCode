...
def predLine(xLine, yLine, Dx, Dy, xc, yc, ome):
    r = np.sqrt((xLine - xc)**2/Dx + (yLine - yc)**2/Dy)
    return -r*np.sqrt(ome/2)
    
# Define geometry
wid = 1
heis = [1, .5, .2]
# Frequency of measurement
omegas = [10, 100, 1000, 10000]
...
for j in range(0, len(heis)):
    hei = heis[j]
    bBox = Box(Point(-wid/2, -wid/2, -hei/2), Point(wid/2, wid/2, hei/2))
    geometry = bBox
    # Generate mesh
    ...
    for i in range(0, len(omegas)):
        ...
        xLine = np.linspace(-wid/2, wid/2, 100)
        yLine = 0*xLine
    ...
    #    plt.show()
        ...
        predphase = predLine(xLine, yLine, Dx, Dy, truexc, trueyc, omega)
        
        plt.plot(xLine/np.cos(theta), Uphase - predphase)
        plt.show()
        
        if(i == 0):
            PhError = Uphase - predphase
            PhSim = Uphase
        else:
            PhError = np.vstack((PhError, Uphase - predphase))
            PhSim = np.vstack((PhSim, Uphase))
            
    fig = plt.gcf()
    plt.contourf(xLine/np.cos(theta), 2*np.pi*np.sqrt(np.divide(2,omegas)), PhError, 100, cmap = cmx.jet)
    plt.colorbar()
    ...
    if(j == 0):
        ...
        fig = plt.gcf()
        plt.contourf(xLine/np.cos(theta), 2*np.pi*np.sqrt(np.divide(2,omegas)), PhError - BaseError, 100, cmap = cmx.jet)
        plt.colorbar()
        plt.title('Depth Difference in Error')
        plt.xlabel('x-coordinate (u)')
        plt.ylabel('Lambda (u)')
        plt.show()
        
        fig = plt.gcf()
        plt.contourf(xLine/np.cos(theta), 2*np.pi*np.sqrt(np.divide(2,omegas)), PhSim - BasePhase, 100, cmap = cmx.jet)
        plt.colorbar()
        plt.title('Depth Difference in Phase')
        plt.xlabel('x-coordinate (u)')
        plt.ylabel('Lambda (u)')
        plt.show()