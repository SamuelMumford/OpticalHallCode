...
def solver(g, tol, mesh, omega, alpha, W):
...
def fullSim(mesh, omega, Dx, Dy, Dz, depth, truexc, trueyc, ktrue, fd, rat):
    ...
    tol = 1E-14
    ureal, uimag = solver(g, tol, mesh, omega, alpha1, W)
...
# Angle of slice through Gaussian beam
theta = 1.7
...
for i in range(len(omegas)):
    ...
    if(np.cos(theta) > .1):
        ds = (xLine - truexc)/np.cos(theta)
    else:
        ds = (yLine - trueyc)/np.sin(theta)
    ...
    if(i == 0):
        ...
        LAll = np.vstack((LAll, Lphase))
        UAll = np.vstack((UAll, Uphase))
#Plotting Section
...
fig = plt.gcf()
plt.contourf(ds, omegas, LAll, 100, cmap = cmx.jet)
plt.colorbar()
plt.title('Phase')
...
fig = plt.gcf()
plt.contourf(ds, omegas, UAll, 100, cmap = cmx.jet)
plt.colorbar()
