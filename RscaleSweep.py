...
# Generate mesh
mesh = generate_mesh(geometry, meshparam)
...
Diff_sm = Uphase - Uphase_0
    

rough = True
#Simulation Section
for i in range(len(Rscales)):
    Rscale = Rscales[i]
    geometry = makeBox(BoxScale, h, NumRs, Rscale, rough)
    # Generate mesh
    mesh = generate_mesh(geometry, meshparam)
...
fig = plt.gcf()
plt.contourf(xLine, omegas, DD, 100, cmap = cmx.jet)
plt.colorbar()
