...
def Ttotal(cutnumx, cutnumy, x, y, z, cutnumz, xc, yx, zc, depth, sign): 
    ...
    for i in range(0,cutnumz):
        ...
        #print('Tr 2D at z = ' + str(z[i]))
        #plt.imshow(T_sheetr, cmap = cmx.jet)
        #plt.colorbar()
        #plt.xlabel('y (points)')
        #plt.ylabel('x (points)')
        #plt.figure()
    ...
    Ttotr = Ttotr/wtot
    Ttoti = Ttoti/wtot
    #Ttotr = np.reshape(Ttotr, (cutnumx, cutnumy))
    #Ttoti = np.reshape(Ttoti, (cutnumx, cutnumy))
    return Ttotr, Ttoti
...
#plt.plot(xline[1:], yline[1:], c = 'black', linewidth = 3)
plt.colorbar()
plt.title('Ideal Phase - Sim Phase (Adjusted by n*2pi)')
plt.xlabel('x')
plt.ylabel('y')
plt.show()
...
#plt.plot(xline[1:], yline[1:], c = 'black', linewidth = 3)
plt.colorbar()
plt.title('Ideal Phase - Base Guess (Adjusted by n*2pi)')
plt.xlabel('x')
plt.ylabel('y')
plt.show()