...
def makeBox(BoxScale, h, NumRs, Rscale, rough):
    if(rough):
        bBox = Box(Point(-(BoxScale- Rscale)/2, -(BoxScale - Rscale)/2, -(h)/2), Point((BoxScale- Rscale)/2, (BoxScale - Rscale)/2, (h)/2))
    else:
        bBox = Box(Point(-(BoxScale)/2, -(BoxScale)/2, -(h)/2), Point((BoxScale)/2, (BoxScale)/2, (h)/2))
    geometry = bBox
    wid = BoxScale/(2*NumRs)
    if(rough):
        for i in range(1, NumRs):
            xst = -BoxScale/2 - Rscale/2 + 2*wid*i
            xBox = Box(Point(xst - wid/2, -(BoxScale + Rscale)/2, -(h)/2), Point(xst + wid/2, (BoxScale + Rscale)/2, (h)/2))
            geometry += xBox
        for i in range(1, NumRs):
...
# Generate mesh
mesh = generate_mesh(geometry, 45)
